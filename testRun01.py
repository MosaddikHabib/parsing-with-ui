...
class PageOne(tk.Frame):
    ...
    def send_data_to_api(self):
        api_link = self.controller.serial_params.get ("api_link")
        if not api_link:
            messagebox.showerror ("API Link Missing", "Please provide an API link in settings.")
            return
        ...
        try:
            cursor.execute ("SELECT * FROM astm_data WHERE sent_to_api = 0")  # Fetch records not sent to API
            records = cursor.fetchall ()
            ...
            for record in records:
                ...
                response = requests.post (api_link, data=json.dumps (parsed_data), headers=headers)

                if response.status_code == 200:
                    cursor.execute ("UPDATE astm_data SET sent_to_api = 1 WHERE sample_id = ?", (sample_id,))
                    connection.commit ()
                    print (f"Data for {sample_id} sent successfully.")
                else:
                    print (f"Failed to send data for {sample_id}. Status code: {response.status_code}")
